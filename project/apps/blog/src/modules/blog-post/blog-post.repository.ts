...
import { PaginationResult, IPost } from '@project/core';
...
export class BlogPostRepository extends BasePostgresRepository<BlogPostEntity, IPost> {
  ...
  public async update(entity: BlogPostEntity): Promise<void> {
    ...
    await this.client.post.update({
      ...
      // Присоединяем связанные таблицы. Без присоединения таблиц мы получим вместо категорий лишь идентификаторы.
      include: {
        categories: true,
        comments: true,
      }
